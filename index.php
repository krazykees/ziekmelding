...
if (!isset($_SESSION['login_id'])) {
    if (isset($_POST['submit'])) {
        ...
        if (!empty($user_username) && !empty($user_password)) {
            ...
            $query = "SELECT name, personell_nr, login_id, dept_nr ,zm_role FROM care_users WHERE personell_nr = '$user_username' AND password = MD5('$user_password')";
            ...
            if (mysqli_num_rows($data) == 1) {
                ...
                $_SESSION['dept_nr'] = preg_match('/"([^"]+)"/', $row['dept_nr'], $afdeling);
                ...
                setcookie('dept_nr', $row['dept_nr'], time() + (60 * 60 * 24 * 30)); // expires in 30 days
                sessie_verlopen();
...
if (empty($_SESSION['personell_nr'])) {
    ...
    <div class="container">
        ...
        <form class="form-signin" role="form" method="post" autocomplete="off" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            ...
            <input required autofocus type="number" name="username" class="form-control" placeholder="Personeel nummer" value="<?php if (!empty($user_username)) echo $user_username; ?>" pattern="^[0-9]*$">
